...
module.exports = {
  ...
  add_current_file: function () {
    ...
    if (!editor || !_.isFunction(editor.getPath)) {
      message_action.warn("Error adding current file: No active text editor. Currently, this feature only works with text files. Sorry.");
      return;
    }
    const p = editor.getPath();
    if (!utils.is_shared(p)) {
      message_action.warn(`Error adding current file: Can't share ${p} because it's not in ${fl.base_path}`);
      return;
    }
