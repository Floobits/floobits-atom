...
module.exports = {
  ...
  on_room_info: function (workspace) {
    ...
    atom.packages.activatePackage('tree-view').then(function (treeViewPkg) {
      const treeView = treeViewPkg.mainModule;
      const tlv = require("./build/terminal-list-view")({terminals: this.terminals});
      const wrapper = require("./react_wrapper");
      const node = wrapper.create_node("terminal-list-view", tlv, {});
      treeView.treeView.find(".tree-view-scroller").prepend(node);
    }.bind(this));
    process.nextTick(function () {
      require("./common/editor_action").create_term();
    });
  },
  ...
  leave_workspace: function () {
    ...
    $("floobits-terminal-list-view").remove();
    if (this.context_disposable) {
