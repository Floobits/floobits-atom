...
module.exports = {
  ...
  activate: function () {
    ...
    atom.commands.add("atom-workspace", {
      ...
      "Floobits: Follow Changes": that.follow.bind(that),
      "Floobits: User List Panel": that.user_list.bind(that),
      "Floobits: Add Current File": that.add_current_file.bind(that),
      "Floobits: Add File To Floobits": that.add_file_from_menu.bind(that),
    });
    atom.contextMenu.add({
      "atom-workspace": [
        {label: "Add to Floobits", command: "Floobits: Add File To Floobits"},
      ]
    });
  ...
  add_file_from_menu: function () {
     const treeView = atom.views.getView(atom.workspace).querySelector('.tree-view');

     if (!treeView) {
       console.log('no treeview');
       return;
     }
    const selectedListItem = treeView.querySelector('.list-tree .selected')
    if (!selectedListItem) {
      return;
    }
    const path = selectedListItem.getPath();
    if (!path) {
      return;
    }
    ...
    fs.readFile(path, function (err, data) {
      ...
      this.addBuf(data.toString(encoding), path, encoding);
    }.bind(this));
  },
  add_current_file: function () {
    ...
    this.addBuf(text, editor.getPath(), "utf8");
  },
