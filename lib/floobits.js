  ...
  add_file_from_menu: function (event) {
    ...
    for (let p of paths) {
      ...
      if (fs.isFileSync(p)) {
        ...
        message_action.error(`Error adding ${p}: Path is neither a file nor a directory. Maybe it's a symlink?`);
        continue;
      }
      /* eslint-enable no-sync */
      this.handler.get_paths_to_upload(p, (err, paths) => {
        if (err) {
          // TODO
          message_action.error(`Error ${err}`);
          return;
        }
        _.each(paths.toUpload, (p) => {
          console.log("reading file", p.path);
          fs.readFile(p.path, (err, data) => {
            // TODO: add feedback on successful addbuf. maybe re-open messages tab?
            if (err) {
              message_action.error(`Error adding ${p.path}: ${err}`);
              return;
            }
            const encoding = utils.is_binary(data, data.length) ? "base64" : "utf8";
            this.addBuf(data.toString(encoding), p.path, encoding);
          });
        });
      });
    }
