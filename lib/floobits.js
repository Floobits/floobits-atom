...
module.exports = {
  ...
  join_workspace_: function (path, url, created) {
    ...
    api.get_workspace(parsed_url.host, parsed_url.owner, parsed_url.workspace, function (err, workspace) {
      ...
      this.handler = new FlooHandler(floourl, this.me, this.users, this.bufs, this.terminals, this.filetree, created);
      this.handler.start();
  ...
  leave_workspace: function () {
    ...
    message_action.log("Leaving " + this.handler.floourl.toString(), true);
    this.handler.stop();
    this.handler = null;
    if (this.userPanel) {
      this.userPanel.destroy();
