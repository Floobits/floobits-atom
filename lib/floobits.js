...
module.exports = {
  ...
  activate: function () {
    ...
    function onConfigUpdate (canJoin, config) {
      try {
        ...
        if (!dirs.length || !pathToFloobits) {
          return;
        }
        const dir = _.find(dirs, function (d) {
          // TODO: normalize paths before compare
          return d.path === pathToFloobits;
        });
        if (!dir) {
          return;
        }
  ...
  join_workspace_: function (path, url, created) {
    ...
    if (root.getPath() !== path) {
      throw new Error("floobits workspace path is not equal to atom's project's root's path.\n" + root.getPath() + " !== " + path);
    ...
    api.get_workspace(parsed_url.host, parsed_url.owner, parsed_url.workspace, function (err, workspace) {
      ...
      that.handler = new FlooHandler(root, floourl, that.me, that.users, that.bufs, that.terminals, that.filetree, created);
      that.handler.start();
