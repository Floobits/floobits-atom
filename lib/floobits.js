...
module.exports = {
  ...
  on_request_perms: function (data) {
    ...
    console.log("User", user, "asked for request_perms", data);

    const HandleRequestPerm = require("./build/handle_request_perm");
    const that = this;
    const view = HandleRequestPerm({username: user.username});
  ...
  join_workspace: function () {
    const JoinWorkspace = require("./build/join");
    const that = this;
    let dotFloo, url;
    ...
    const view = JoinWorkspace({url: url, on_url: function (_path, _url) {
      if (!_url) {
