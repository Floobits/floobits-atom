...
module.exports = {
  ...
  activate: function () {
    ...
    atom.commands.add(".tree-view .selected", "Floobits: Add to Workspace", that.add_file_from_menu.bind(that));
  ...
  on_room_info: function (workspace) {
    ...
    const that = this;
    // I'm ashamed of this - room_info CB order is non deterministic (need users...)
    process.nextTick(function () {
      _.each(terms, function (term, termId) {
        const user = that.users.getByConnectionID(term.owner);
        that.terminals.addTerminal(term.id, term.term_name, term.size[0], term.size[1], user.id);
      });
    });

    this.context_disposable = atom.contextMenu.add({
      ".tree-view .selected": [
        {
