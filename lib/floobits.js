...
module.exports = {
  ...
  activate: function () {
    ...
    atom.commands.add("atom-workspace", {
      ...
      "Floobits: Refresh Workspace": this.refresh_workspace.bind(this),
  ...
  create_or_set_: function (file, cb) {
    const fluffer = this.bufs.findFluffer(file);
    let atomBuffer;
    if (fluffer) {
      atomBuffer = this.atom_listener.atomBuffers[fluffer.id];
    }
    const utils = require("./common/utils");
    const floop = require("./common/floop");
    /* eslint-disable no-sync */
    ...
    /* eslint-enable no-sync */
    if (atomBuffer && atomBuffer.isAlive) {
      const atomText = atomBuffer.getText();
      const md5 = utils.md5(atomText);
      const message_action = require("./common/message_action");
      if (fluffer.md5 === md5) {
        message_action.info(`Skipping ${p} because md5 matches`);
        cb();
        return;
      }
      floop.send_set_buf({
        ...
        buf: atomText,
        md5,
        encoding: fluffer.encoding,
      }, cb);
      // 🍆🍆🍆
      fluffer.set({buf: atomText, md5}, {silent: true});
      return;
    }
    if (!utils.is_shared(p)) {
      cb(`Can't share file because it's not in ${fl.base_path}`);
      return;
    }
    const fs = require("fs-plus");
    const path = require("path");
    fs.readFile(p, (err, data) => {
      // TODO: add feedback on successful addbuf. maybe re-open messages tab?
      // TODO: if atom buffer isn't saved, we'll send an out-of-date version of the buf
      ...
      const md5 = utils.md5(data);
      const encodedData = data.toString(encoding);
      ...
      if (fluffer) {
        ...
        fluffer.set({buf: encodedData, md5}, {silent: true});
        return;
      }
    ...
    for (let p of paths) {
      ...
      this.handler.get_paths_to_upload(p, (err, paths) => {
        ...
        async.eachLimit(paths.toUpload, 10, (file, cb) => {
          this.create_or_set_(file, (err, res) => {
            /* eslint-disable no-sync */
            const file_rel = path.relative(fl.base_path, file.getRealPathSync());
            /* eslint-enable no-sync */
            if (err) {
              message_action.error(`Error adding ${file_rel}: ${err.toString()}`);
            } else {
              message_action.success(`Added ${file_rel}`);
            }
            // Keep on truckin'
            return cb(null, res);
          });
  ...
  refresh_workspace: function () {
    ...
    this.handler.refresh_workspace();
  },
