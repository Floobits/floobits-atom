...
module.exports = {
  ...
  welcome_web_view: function () {
    const V = require("./build/webview");
    const P = require("../templates/pane");
    const view = new V();
    view.component.src = "https://floobits.com";
    const p = new P("Welcome to Floobits", "", view);
    atom.workspace.getActivePane().activateItem(p);
  },
  welcome: function () {
    return this.welcome_web_view();
    const that = this;
    ...
    api.get_workspace(parsed_url.host, parsed_url.owner, parsed_url.workspace, function (err, workspace) {
      if (err) {
        ...
        if (res.statusCode === 404) {
          ...
          return ync("404!", "The workspace " + floourl.toString() + " does not exist.  Do you want to create it?", function (err, res) {
            if (res !== "yes") {
              return;
            }
  ...
  user_list: function () {
    ...
    const users = require("./react_wrapper").create_node('users', view,
      {width: "230px", height: "100%", overflow: "auto"}
    );
    ...
    users.onDestroy(this.userPanel);
  },
