...
module.exports = {
  ...
  on_room_info: function (workspace) {
    ...
    this.context_disposable.add(
      atom.commands.add("atom-workspace", {
        ...
        "Floobits: Permissions": this.permissions.bind(this),
      })
    );
  ...
  code_review: function () {
    ...
    const dialog_cb = function () {
      const prompt = "Would you like some humans to look over your code?";

      return ync("Experimental: Code Review", prompt, function (unused, choice) {
        if (choice !== "yes") {
          return;
        }
        ...
        api.post_code_review(parsed_url.host, parsed_url.owner, parsed_url.workspace, function (err) {
          console.log(err);
        });
      });
    };
    ...
    const review_cb = function (err, res) {
      ...
      const prompt = "Do you want to share this directory so it can be reviewed?";
      return ync("Code Review", prompt, function (unused, choice) {
        ...
        const editorAction = require("./common/editor_action");
        editorAction.onceHANDLE_CONFLICTS(function () {
          dialog_cb();
        });
        return;
      });
