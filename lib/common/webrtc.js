...
const messageAction = require("./editor/message_action");
const webrtcAction = require("./editor/webrtc_action");
const perms = require("./editor/permission_model");
const prefs = require("./editor/userPref_model");
...
function WebRTC (users, me, isChat) {
  ...
  handlerAction.onREADY(_.bind(function (roomInfo) {
  ...
  handlerAction.on(handlerAction.CONNECTION_JOINED, function (connectionInfo) {
    if (!this.commStream && !this.screenStream) {
      return;
    }
    Socket.send_webrtc({
...
WebRTC.prototype.handleMsg_ = function (msg) {
  ...
  switch (msg.action) {
    ...
    case "start":
      ...
      const chatType = prefs.audioOnly ? "audio" : "video";
...
WebRTC.prototype.closePeer_ = function (connId, type, opt_silent) {
  ...
  setTimeout(function () {
    var still_connected = _.any(["video", "audio", "screen"], function (t) {
...
WebRTC.prototype.startUserMedia = function (type, users) {
  ...
  const mcs = mediaConstraints[type];
  if (!mcs) {
