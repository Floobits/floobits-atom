...
var connConfig,
  ...
  SCREEN_SHARE,
  AUDIO_SHARE,
  getUserMedia = require("./extern/webrtc/get_user_media"),
  webrtcsupport = require("./extern/webrtc/webrtc_support"),
  PeerConnection = require("./extern/webrtc/peer_connection"),
  ExtMessaging = require("./ext_messaging"),
  SoundEffects = require("./sound_effects"),
  // TODO: replace Modal
  Modal = require("../modal"),
  floop = require('./floop'),
...
function WebRTC (users, me, isChat) {
  ...
  floop.onWEBRTC(_.bind(this.handleMsg_, this));
  ...
  // TODO: kans
  // floop.onREADY(_.bind(function (roomInfo) {
  //   if (_.contains(roomInfo.perms, "patch")) {
  //     return;
  //   }
  ...
  floop.onJOIN(function (connectionInfo) {
    if (!this.commStream && !this.screenStream) {
      return;
    }
    floop.send_webrtc({
...
WebRTC.prototype.handlePolo_ = function (otherConnId, streams) {
  var self = this, otherUser, myConnId, msg;
...
WebRTC.prototype.sendReject = function (username, otherUserId) {
  var reason;
  if (prefs.dnd) {
    messageAction.info(`Rejected ${username}’s video chat because you enabled Do Not Disturb.`);
    reason = "Do Not Disturb on";
  } else if (!webrtcsupport.PeerConnection) {
    messageAction.info(`Rejected ${username}’s video chat because your browser doesn’t support WebRTC.`);
...
WebRTC.prototype.handleClosedConnection_ = function (connId, type) {
  ...
  conn = this.users.getConnectionByConnectionID(connId);
  conn.stopStream();
  if (type === "screen") {
...
WebRTC.prototype.addStream_ = function (connection, stream, type) {
  var audioSource, analyser, jsProcessor, context;
...
/**
 * This function handles messages intended for WebRTC. It's a
 * general handler for all data messages and functions as a switch.
 * @param {Object} data
 * @private
 */
WebRTC.prototype.handleMsg_ = function (msg) {
  var down_pc, up_pc, pcKey, supportedMessages, data, user, username, conn, rejectMsg;
  ...
  if (!_.contains(supportedMessages, msg.action)) {
  ...
  switch (msg.action) {
    ...
    case "unmute":
      ...
      break;
    case "reject":
      rejectMsg = `${username} rejected your video chat request.`;
      if (data) {
    ...
    case "start":
      if (prefs.dnd || !webrtcsupport.PeerConnection) {
        this.sendReject(username, msg.user_id);
      } else {
        if (this.commStream) {
          floop.send_webrtc({
            action: "marco",
          ...
          this.startUserMedia(prefs.audioOnly ? "audio" : "video");
        }
      }
      break;
    case "stop":
      if (_.contains(data, "video")) {
        this.stopUserMedia(this.commStream, "video");
      }
      if (_.contains(data, "audio")) {
        this.stopUserMedia(this.commStream, "audio");
      }
      if (_.contains(data, "screen")) {
...
WebRTC.prototype.processOfferSDP_ = function (userId, type, offer) {
  var pc = this.setupPeer_(this.up_peers_, userId, type);
...
WebRTC.prototype.closePeer_ = function (connId, type, opt_silent) {
  ...
  silent = _.isUndefined(opt_silent) ? false : opt_silent;
  if (down_pc && !silent) {
  ...
  setTimeout(function () {
    still_connected = _.any(["video", "audio", "screen"], function (type) {
      return !!self.down_peers_[connId + "_" + type];
    });
    ...
    _.each(["video", "audio", "screen"], function (type) {
      var pcKey = connId + "_" + type,
...
WebRTC.prototype.startScreen = function () {
  ...
  if (this.screenStream) {
    // TODO: this starts screen sharing with everyone. we should probably just send to ppl we're video chatting with
    floop.send_webrtc({
      action: "start",
      data: this.myStreams(),
      to: [],
...
/**
 * Starts video/audio chat or screen share.
 */
WebRTC.prototype.startUserMedia = function (type, users) {
  var mcs = mediaConstraints[type];
  if (!mcs) {
...
WebRTC.prototype.stopStream = function (stream, type) {
  ...
  _.each(this.up_peers_, function (pc, pcKey) {
...
WebRTC.prototype.stopUserMedia = function (stream, type) {
  ...
  function closePeers (peers) {
    ...
    _.each(peers, function (peer, k) {
      ...
      try {
        self.closePeer_(userId, type, true);
      } catch (e) {
        console.debug(e);
      }
    });
