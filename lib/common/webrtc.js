...
function WebRTC (users, me, isChat) {
  ...
  Socket.onROOM_INFO(_.bind(function (roomInfo) {
  ...
  Socket.onJOIN(function (connectionInfo) {
    if (!this.commStream && !this.screenStream) {
      return;
    }
    Socket.send_webrtc({
...
WebRTC.prototype.startUserMedia = function (type, users) {
  ...
  if (prefs.source_audio_id && mcs.audio) {
    mcs.audio.optional.push({ sourceId: prefs.source_audio_id });
  }

  if (mcs.video) {
    if (prefs.source_video_id) {
      mcs.optional.push({ sourceId: prefs.source_video_id });
    } else {
      mcs.optional.push({ facingMode: "user" });
    }
  }
