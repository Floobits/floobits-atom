...
const context = self.AudioContext && new self.AudioContext();
...
const Connection = flux.createModel({
  ...
  screenStream: null,
  isWeb: function () {
    return _.contains(utils.BROWSER_CLIENTS, this.client);
  },
...
Users.prototype.connsForPerm = function (name, perm) {
  return _.chain(this.data.collection)
    .flatten("connections")
    .filter(function (conn) {
      return !conn.isMe && (conn.client === "web" || "atom" ) && conn.perms.indexOf(perm) > -1;
    })
...
Users.prototype.broadcast_data_message_for_perm = function (datamsg, perm) {
  const ids = _.chain(this.data.collection)
    .flatten("connections")
    .filter(function (conn) {
      return !conn.isMe && conn.client === "web" && conn.perms.indexOf(perm) > -1;
    })
    .map(function (conn) { return conn.id; })
    .value();
  ...
  ids.forEach(function (id) {
    floop.emitDataMessage(datamsg, id);
  });
};
