...
FlooHandler.prototype.on_room_info = function (workspace) {
  ...
  async.auto({
    ignores: function (cb) {
      that.ignores.init(that.directory, cb);
    },
    files: ["ignores", function (cb, res) {
      const files = [];
      const entries = [that.directory];
      let entry;
      let totalSize = 0;
      const ignores = that.ignores;
      const dir_path = that.directory.getPath();
      while (entries.length) {
        ...
        if (entry.isFile()) {
          const size = ignores.getSize(p);
          if (ignores.is_too_big(size)) {
            tooBig[p] = size;
            console.log("too big", p);
            continue;
          }
          totalSize += size;
          ignores.add_ignore(entry);
          files.push(entry);
          continue;
        }
