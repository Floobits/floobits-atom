...
function FlooHandler(directory, floourl, me, users, bufs, terminals, filetree, createdWorkspace) {
  this.get_buf_cb = {};
  this.paths_to_ids = {};
...
FlooHandler.prototype.on_room_info = function (workspace) {
  ...
  async.auto({
    repo: function (cb) {
      atom.project.repositoryForDirectory(that.directory).then(cb, cb);
    },
    files: ["repo", function (cb, res) {
      const files = [];
      const repo = res.repo;
      ...
      while (entries.length) {
        ...
        const gitIgnored = repo && repo.isPathIgnored(p) && p !== that.directory.getPath();
