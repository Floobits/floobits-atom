...
FlooHandler.prototype.create_or_set = function (file, cb) {
  const fluffer = this.bufs.findFluffer(file);
  if (fluffer) {
    // TODO: check md5s and skip this upload if they're equal
    floop.send_set_buf({
      id: fluffer.id,
      buf: fluffer.buf,
      md5: fluffer.md5,
    ...
    if (!utils.is_shared(p)) {
      messageAction.error(`Error adding ${p}: Can't share file because it's not in ${fl.base_path}`);
      setImmediate(cb);
      return;
    }
    fs.readFile(p, (err, data) => {
      ...
      if (err) {
        messageAction.error(`Error adding ${rel}: ${err}`);
        return;
      }
...
FlooHandler.prototype.on_room_info = function (workspace) {
  ...
  async.auto({
    ...
    const ignored = res.paths && res.paths.ignored;
    const tooBig = res.paths && res.paths.tooBig;
