...
FlooHandler.prototype.start = function () {
  ...
  perms.on(mugHandler);
  prefs.on("dnd", mugHandler);
  prefs.on("mugshots", mugHandler);
...
FlooHandler.prototype.on_room_info = function (workspace) {
  ...
  const is_ignored = function (file) {
    const p = file.getPath();

    if (that.flooIgnore.is_too_big(null, p)) {
      return true;
    }
    if (that.flooIgnore.is_unignored(null, p)) {
      return false;
    }
    if (repo && repo.isPathIgnored(p)) {
      return true;
    }
    if (that.flooIgnore.is_ignored(null, p)) {
      return true;
    }
    return false;
  };
  ...
  async.auto({
    files: function (cb) {
      const files = [];
      let entries = [atom.project.getRootDirectory()];
      let entry;
