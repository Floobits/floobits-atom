...
AtomListener.prototype.on_floobits_highlight = function (data, recursive) {
  ...
  let klass = "floobits-highlight-dark_" + user.color;
  ...
  _.each(pane.items, function (item) {
    if (_.includes(toActivate, item.id)) {
...
AtomListener.prototype.setText = function (text, id, buffer) {
  ...
  // TODO: better newline detection
  if (buffer.lineEndingForRow(0) === "\r\n") {
    // Windows newlines
    text = text.replace("\n", "\r\n");
  }
...
function crOffset(atomBuffer, patch) {
  if (atomBuffer.lineEndingForRow(0) !== "\r\n") {
    return patch;
  }
  let offset = patch[0];
  const buffer = atomBuffer.getText().slice(offset);
  for (let c in buffer) {
    if (c === "\r") {
      offset++;
    }
  ...
  patch[0] = offset;
  patch[2] = patch[2].replace(utils.LFRegex, "\r\n");
  // TODO: not sure what length is for deletes
  patch[1] = patch[2].length;
  return patch;
}
...
AtomListener.prototype.on_fluffer_changed = function (b, charPointer, patches, username) {
  ...
  try {
    ...
    console.error(e);
    messageAction.error(`Error applying patch to ${b.path}: ${e.toString()}`, false);
  }
  this.ignore_events = false;
};
...
AtomListener.prototype.send_delete_buf = function (bufferID, unlink) {
  ...
  floop.send_delete_buf({id: bufferID, unlink: unlink});
};

AtomListener.prototype.start_chokidar_ = function () {
  const that = this;
  ...
  if (!atom.config.get("floobits.fs_watch")) {
    return;
  }
  ...
  if (process.platform === "darwin") {
    let can_watch = false;
    try {
      // Chokidar doesn't expose a way to know if we're polling or not. Hacky, but necessary.
      can_watch = !!chokidar.FSWatcher.prototype._addToFsEvents;
    } catch (unused) {
      // Ignored
    }
    if (!can_watch) {
      console.warn("No FSEvents watcher available. Files changed outside of Atom may not be updated in the workspace.");
      return;
    }
  }
  ...
  this.watcher
    ...
    .on('change', function(path) {
      ...
      if (buf) {
        ...
        if (buf.send_patch([txt.toString(buf.encoding)])) {
      ...
      const encoding = utils.is_binary(txt, txt.length) ? 'base64' : "utf8";
      floop.send_create_buf({
        path: rel,
...
AtomListener.prototype.start = function () {
  ...
  editorAction.onHANDLE_CONFLICTS(function (args) {
    if (_.isEmpty(args.newFiles) && _.isEmpty(args.different) && _.isEmpty(args.missing)) {
      that.start_chokidar_();
      that.handledConflicts = true;
      return;
    }
    args.onHandledConflicts = function (toFetch) {
      that.toFetch = toFetch;
      that.handledConflicts = true;
