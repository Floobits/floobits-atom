...
function AtomListener (bufs, users, me) {
  ...
  this.send_patch_for = {};
  this.highlights_by_username = {};
  this.highlights_by_conn_id = {};
  this.buffersSaved = {};
  this.movedFiles = {};
...
AtomListener.prototype.on_floobits_highlight = function (data) {
  var fluffer, editors, markers, highlights = this.highlights_by_conn_id;
  ...
  _.each(editors, function (editor) {
    ...
    _.map(data.ranges, function (range) {
      ...
      highlights[user_id].push(marker);
    });
  });
  this.lastHighlightReceived = data;
  this.highlights_by_username[data.username] = data;
...
AtomListener.prototype.stop = function () {
  _.each(this.highlights_by_username, function (markers) {
    markers.forEach(function (m) {
      try {
        m.destroy();
      } catch (e) {
...
AtomListener.prototype.start = function () {
  ...
  editorAction.onJUMP_TO_USER(function (username) {
    ...
    if (username === "floobits") {
      ...
      data = this.highlights_by_username[username];
    }
    if (!data) {
      return;
    }
  ...
  bufferAction.onCREATED(function (buf, username) {
    const isMe = username === that.username;
    let buffer = that.atomBuffers[buf.id];
    if (buffer) {
      throw new Error("Create buf for existing buffer:", buf.id);
    }
