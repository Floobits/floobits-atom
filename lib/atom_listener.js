...
AtomListener.prototype.onDidChangeSelectionRange = function (editor) {
  ...
  let fluffer = this.bufs.findFluffer(editor);
  ...
  floop.send_highlight({
    ...
    'ranges': _.map(selections, function (selection) {
      ...
      let start = editor.buffer.characterIndexForPosition(range.start);
      let end = editor.buffer.characterIndexForPosition(range.end);
      return [start, end];
    })
  });
};
