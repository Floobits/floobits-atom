...
AtomListener.prototype.on_floobits_highlight = function (data) {
  ...
  _.each(editors, function (editor) {
    var buffer, offsetIndex, ranges, marker;
    ...
    ranges = _.map(data.ranges, function (range) {
    ...
    // marker = buffer.markRange(ranges[0]);
    marker = editor.markBufferRange(ranges[0], {invalidate: 'never'});
    // TODO: remove old ones....
    console.log(ranges[0]);
    editor.decorateMarker(marker, {type: "highlight", class: "floobits-highlight"});
  });
...
AtomListener.prototype.start = function () {
  ...
  bufferAction.onCHANGED(function (b) {
    ...
    if (buffer && buffer.isAlive()) {
      ...
      try {
        // TODO: we can make this async if we check the onchange handler if a buffer exists
