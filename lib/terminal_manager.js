...
TerminalManager.prototype.start_ = function () {
  ...
  const disposable = that.term3.onTerm(function (t) {
    const d = t.getDimensions();
    Socket.send_create_term({term_name: that.nameForTerm(t), size: [d.cols, d.rows]});
  });

  this.subs.add(disposable);

  that.term3.getTerminals().forEach(function (t) {
    const d = t.getDimensions();
    Socket.send_create_term({term_name: that.nameForTerm(t), size: [d.cols, d.rows]});
  });

  _.each(this.floobits_terms, function (t, termID) {
...
TerminalManager.prototype.stop = function( ) {
  if (this.subs) {
    this.subs.dispose();
    this.subs = null;
  }
