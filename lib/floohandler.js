...
FlooHandler.prototype.on_patch = function (d) {
  ...
  var abs_path = path.join(self.base_path, buf.path),
  ...
  _.each(result[2], function (patch) {
    ...
    console.log("replacing", start_pos, end_pos, replace);
        var a = editor.buffer.getText();
    editor.buffer['setTextInRange'](new Range(start_pos, end_pos), replace);
    // var b = editor.buffer.getText();
    // editor.buffer.insert(start_pos, replace);
    var c = editor.buffer.getText();
