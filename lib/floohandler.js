  ...
  async.auto({
    ...
    editors: function (cb) {
      atom.workspace.eachEditor(function (editor) {
        ...
        different[fluffer.id] = fluffer.path;
      });
      cb(null);
    },
    ...
    let conflicts = wrapper.create_node('conflicts', {}, view({newFiles: newFiles, different: different, missing: missing}), 
      {width: "100%", height: "100%", overflow: "auto", color: "white"});
    
    atom.workspace.getActivePane().activateItem(conflicts);
    // _.each(ids, function (buf, id) {
    //   that.bufs.get(id).shouldSave = true;
