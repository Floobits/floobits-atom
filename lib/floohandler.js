...
function FlooHandler(floourl, floorc) {
  ...
  this.listener = new Listener(this.bufs, this.users);
...
FlooHandler.prototype.on_room_info = function (workspace) {
  ...
  _.each(workspace.bufs, function (buf, id) {
    this.bufs.add(buf);
    this.bufs.paths_to_ids[buf.path] = id;
  }, this);
  ...
  async.eachLimit(files, 10, function(file, cb) {
    ...
    if (!fluffer) {
      newFiles[file.getRealPathSync()] = file;
      return cb();
    }
    // already handled
    ...
    file.read().then(function (data) {
      ...
      if (md5 !== fluffer.md5) {
        different[file.getRealPathSync()] = fluffer.id;
      } else {
        found[file.getRealPathSync()] = file;
      }
      return cb();
    });
  }, function (err) {
    console.log(newFiles, different, found);

    _.each(different, function (id) {
      floop.send_get_buf(id);
    });

    _.each(newFiles, function (file, p) {
      floop.send_create_buf({
        buf: file.readSync(),
        path: p,
        // TODO: handle binary shit
        encoding: "utf8",
      });
    });
  });
...
FlooHandler.prototype.get_buf_by_path = function (_path) {
  var p = path.relative(fl.base_path, _path),
